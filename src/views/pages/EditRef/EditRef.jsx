...
const EditRef = () => {
    ...
    const loadUser = async () => {
        const resp = await axios.get(`http://13.212.153.21:3000/aggregateemotionrefobjective?refObjectiveId=${id}`);
        console.log(resp);
    ...
    const handleSubmit = (e) => {
        ...
        console.log(Data);
        axios.post('http://13.212.153.21:3000/saverefobjective', Data)
        .then(response => {
            ...
            <div className="container justify-content-around h-100">
                ...
                <div className="container border h-100 w-50 d-inline-block mt-3 bg-white">
                    <div className="refObjective row" id='refObjective' >
                            <p className='p-0 mx-2 mt-2'><b>Reference objective <span>{details.title}</span></b></p>
                                    ...
                                    return (
                                        <>
                                            <div key={index} className='container w-100 justify-content-between'>
                                                <p key={item.objectiveName}><b>{item.objectiveName}</b></p>
                                                <p key={item.id}><b>{item.score ? item.score.toFixed(1) : "null"}%</b></p>
                                            </div>
                                                        ...
                                                        <div className='inputBtn justify-content-center'>
                                                            <li>
                                                                <input type="button"
                                                                    ...
                                                                    key={sub.emotionId} id='input' name={sub.emotionId} value=''
                                                                />
                                                            </li>
                                                            <li>
                                                                <label htmlFor={sub.emotionId} key={sub.emotionId} className='text-center'><b>{sub.emotionId}</b></label>
                                                            </li>
                                                            <li>
                                    ...
                                    return (
                                        <div key={index}>
                                            <div className="mb-3 mx-0 p-3 bg-light" id='subObjective'>
                                                <label className="" ><b>Sub objective</b></label>
                                                <IconButton >
                                                ...
                                                />
                                                <p className="m-3"><b>Add Behaviours/Emotions to be measured</b></p>
                                                <div className="container" id="div1">
                                                    <div className='container bg-white' id={index} onDrop={event => drop(event,index)} onDragOver={event => allowDrop(event)}>

                                                    </div>
                                                    <div>
                                                        <button className="btn border border-dark rounded-circle float-end mx-2"
                                                            name='associatedEmotions' type='button' onClick={event => AddEmotions(index,event)}>+</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className=' border-bottom'>
                        <button className="btn border border-dark border-2 float-end"
                         id="subBtn1" onClick={() => AddsubObjective()}>+ Add Sub Objective</button>
                    </div>
                  </div>  
                    <div className='d-flex justify-content-end m-3'>
                        <Link to='/objective2' className='btn btn-primary mx-2'>Cancel</Link>
                        <Link  className="btn btn-warning" onClick={handleSubmit}>Apply Changes</Link>
                    </div>
                </div>
                <div className="container border border-3 w-25 d-inline-block mt-3">
                    <div className='editTable'>
                        ...
                        <div className='mt-4'>
                            <b><p>Flexibility</p></b>
                            <label htmlFor="customRange2" className="form-label">Critically</label>
                            <Slider
                                min={1}
                                max={5}
                                marks={Critically}
                                ...
                                id="customRange2"
                            />
                            <label htmlFor="customRange2" className="form-label">Weightage</label>
                            <Slider
                                min={0}
                                max={100}
                                marks={Weightage}
