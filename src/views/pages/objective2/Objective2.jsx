...
export default function WidgetLg() {
  ...
  const deleteUser = () => {
    ...
    console.log(id);
    axios.post(`http://13.212.153.21:3000/deleterefobjectives`, id)
    .then(response => {
      loadUsers();
    ...
    <div className="widgetTitle">
      <div className="widgetLg1">
        <div className='container title'>
          <i className="titleIcons  p-2 mb-3 shadow bg-white rounded" ><BsPieChart size={30} color="red" /></i>
          <h3 className='mx-3'> Create New Reference Objective</h3>
        </div>
        <div className="container justify-content-end">
          <i className="titleIcons mr-3 shadow p-2 mb-3 bg-dark rounded"><AiFillStar size={30} color="white" /></i>
          <Link className="text-decoration-none px-5 mx-2 btn text-white" to="/objective2/createNewRef" id="btn">Create</Link>
        </div>
      </div>
      <div className='container justify-content-end'>
        {/* <span className='border border-danger'><AiOutlineSearch size={30} /> */}
        <input placeholder='search....' className='border-0 border border-danger' type='text'></input>
      </div>
      <div className="widgetLg">
        ...
        <CTable align="middle" className="mb-0 border" hover responsive>
                <CTableHead color="light">
                  <CTableRow>
                    <CTableHeaderCell className='col-1 text-center' scope="col">#</CTableHeaderCell>
                ...
                {paginatedPosts.map((user,index) => (
                    <CTableRow v-for="item in tableItems" key={index}>
                      ...
                      <input type="checkbox" value={user.id} id="checkbox" className='checkbox'
                        onChange={event => handleId(event)}
                      />
                      </CTableDataCell>
                      <CTableDataCell>
      ...
      <div className='container'>
        <div className="container justify-content-end">
          <button className="btn m-2 bg-white border border-3 border-danger text-danger" 
              id="deleteBtn" 
              onClick={deleteUser}
              disabled={state()}
            >
                <RiDeleteBin5Line color="red" /><b className=' mx-2 pt-1'>Delete</b></button>
          ...
          <nav aria-label="Page navigation example">
            <ul className="pagination">
              <li key='nav_prev' className="page-item">
                <Link className={
                  currentPage === 1 ? "page-link disabled-link" : "page-link"
                } 
                to="#" aria-label="Previous" onClick={() => pagination(currentPage - 1)}>
              ...
              </li>
              {
                pages.map((page,index) => (
                  ...
                  }><Link className="page-link active" to="#" onClick={() => pagination(page)}>{page}</Link></li>
                ))
              }
              <li key='nav_next' className="page-item">
                ...
                to="#" aria-label="Next" onClick={() => pagination(currentPage + 1)}>
